...
libSparkFHE_path=$SparkFHE_distribution/deps/lib:$HADOOP_HOME/lib/native:/usr/local/lib:/usr/local/hadoop/lib/native
...
case $ynq in
	[Yy]* ) 
		...
		run_spark_submit_command  sparkfhe_keygen  spiritlab.sparkfhe.example.nonbatching.KeyGenExample local HELIB BGV;;
    [Nn]* ) 
...
case $ynq in
	[Yy]* ) 
		...
		run_spark_submit_command  sparkfhe_encryption_decryption  spiritlab.sparkfhe.example.nonbatching.EncDecExample local HELIB BGV \
...
case $ynq in
	[Yy]* ) 
		run_spark_submit_command  sparkfhe_basic_OPs_examples  spiritlab.sparkfhe.example.nonbatching.BasicOPsExample local HELIB BGV \
			"gen/keys/my_public_key.txt" \
			"gen/keys/my_secret_key.txt";;
    [Nn]* ) 
		echo "Skip to the next job.";;
...
case $ynq in
	[Yy]* ) 
		run_spark_submit_command  sparkfhe_dot_product_examples  spiritlab.sparkfhe.example.nonbatching.DotProductExample local HELIB BGV \
			"gen/keys/my_public_key.txt" \
			"gen/keys/my_secret_key.txt";;
    [Nn]* ) 
		echo "Skip to the next job.";;
...
case $ynq in
	[Yy]* ) 
		run_spark_submit_command  sparkfhe_total_sum_examples  spiritlab.sparkfhe.example.nonbatching.TotalSumExample local HELIB BGV \
			"gen/keys/my_public_key.txt" \
			"gen/keys/my_secret_key.txt";;
    [Nn]* ) 
		echo "Skip to the next job.";;
